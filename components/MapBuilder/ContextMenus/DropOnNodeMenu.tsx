...
const DropOnNodeMenu: React.FC<DropOnNodeMenuProps> = ({ menuState, onAttach, onAppend, onClose, onClearNotesToPlace }) => {
    return (
        <div 
        ...
        >
            <button 
                onClick={() => { 
                    onAttach(menuState.targetNodeId, menuState.droppedNotes); 
                    onClearNotesToPlace();
                    onClose(); 
                }}
            ...
            <button 
                onClick={() => { 
                    onAppend(menuState.targetNodeId, menuState.droppedNotes);
                    onClearNotesToPlace();
                    onClose(); 
                }}
                className="block w-full text-left px-3 py-1.5 hover:bg-gray-700 rounded flex items-center gap-2"
