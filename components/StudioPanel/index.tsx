...
const EditableNoteCard: React.FC<EditableNoteCardProps> = ({ note, onSave, onDelete, onClose, onAiRequest, ai, logActivity, nodeName, allProjectNotes, onNavigateToNote, tags }) => {
    ...
    const handleExport = () => {
        ...
        const noteTags = tags ? note.tagIds?.map(id => tags.find(t => t.id === id)?.name).filter(Boolean).join(', ') : '';
        
        const styles = `
            body { background-color: #111827; color: #e5e7eb; font-family: 'Inter', sans-serif; padding: 2rem; max-width: 800px; margin: 0 auto; }
            h1 { color: #f3f4f6; border-bottom: 1px solid #374151; padding-bottom: 0.5rem; }
            .metadata { font-size: 0.875rem; color: #9ca3af; margin-bottom: 2rem; }
            .content { line-height: 1.6; }
            .nexus-link { background-color: rgba(6, 182, 212, 0.15); border: 1px solid #0891b2; color: #a5f3fc; padding: 0.1rem 0.4rem; border-radius: 0.5rem; font-weight: 500; text-decoration: none; }
            .weblink { color: #93c5fd; text-decoration: underline; text-decoration-style: dotted; text-underline-offset: 2px; }
            blockquote { border-left: 4px solid #38bdf8; padding-left: 1rem; margin: 1rem 0; color: #d1d5db; background-color: rgba(55, 65, 81, 0.3); padding: 0.5rem 1rem; border-radius: 0.25rem; }
            .latex-inline { font-family: 'Times New Roman', serif; font-style: italic; color: #a7f3d0; }
            /* Tailwind Prose Basics for exported context */
...
<body>
    ...
    <div class="metadata">
        ...
        ${noteTags ? `<p><strong>Tags:</strong> ${noteTags}</p>` : ''}
    </div>
    <div class="content">
    ...
    return (
        <div className="flex-grow flex flex-col h-full relative editable-note-card-root">
            ...
            <div className="flex-shrink-0 px-3 py-2 border-t border-gray-700 bg-gray-900/70 flex items-center justify-between">
                <span className="text-xs text-gray-400">
                    <span className="font-semibold text-gray-300">{wordCount}</span> {wordCount === 1 ? 'word' : 'words'}
                </span>
            </div>
